...
times <- strptime(paste(powerReadings$Date,powerReadings$Time), 
                  format="%d/%m/%Y %H:%M:%S")
power <- powerReadings$Global_active_power
Voltage <- powerReadings$Voltage
reactive <- powerReadings$Global_reactive_power
...
par(mfrow=c(2,2))
plot(times, power,type="l",ylab="Global active power",xlab="", col="black")
plot(times, Voltage,type="l",ylab="Voltage",xlab="datetime", col="black")
...
plot(times, reactive,type="l",ylab="Global_reactive_power",xlab="datetime")
dev.off()